<?php
	...
	if ($_SERVER['HTTP_USER_AGENT'] != "Graphictoria3") {
		echo 'game:SetMessage("Not being called from a Graphictoria server.");';
		$dbcon = null;
		exit;
	}
	if (isset($_GET['key'])) {
...
coroutine.resume(coroutine.create(function()
	while (true) do
		<?php
			if (!$dedicatedServer) echo 'game:HttpGet("http://api.xdiscuss.net/server/ping.php?key='.$serverKey.'&tick=" .. tick(), true)';
			if ($dedicatedServer) echo 'game:HttpGet("http://api.xdiscuss.net/server/ping.php?key='.$serverKey.'&players=" .. game.Players.NumPlayers .. "&tick=" .. tick(), true)';
		?>
		wait(60)
	end
end))
...
function authenticate(newPlayer)
	coroutine.resume(coroutine.create(function()
		local verification = game:HttpGet("http://api.xdiscuss.net/server/checkPlayer.php?uname=" .. newPlayer.Name .. "&gameId=<?php echo $serverID;?>&uid=".. newPlayer.userId .."&tick=" .. tick(), true)
...
game:GetService("Players").PlayerAdded:connect(function(newPlayer)
	...
	if (authenticate(newPlayer)) then
		<?php if ($gameVersion == 0 || $gameVersion == 2) {
			echo 'newPlayer.CharacterAppearance = "http://api.xdiscuss.net/user/getCharacter.php?uid=" .. newPlayer.userId .. "&key=D869593BF742A42F79915993EF1DB&tick=" .. tick()';
		}?>
